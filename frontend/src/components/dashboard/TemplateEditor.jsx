...
export default function TemplateEditor({ templateId, onBack, token, onTemplateSaved }) {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      setIsLoading(true);
      try {
        // Only fetch media files, as we might be creating a new template
        ...
        if (isNewTemplate) {
          // If it's a new template, create a blank structure
          setTemplate({
            name: 'New Template',
            segments: [],
          ...
          // If editing, fetch the specific template
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      onTemplateSaved(); // This will trigger a refresh in the manager
    } catch(err) {
      setError(err.message);
    } finally {
      setIsSaving(false);
  ...
  // (The renderSegmentEditor function and the main return JSX remain the same as your original file)
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-4">
        <Button onClick={onBack} variant="ghost"><ArrowLeft className="w-4 h-4 mr-2" />Back to Template List</Button>
