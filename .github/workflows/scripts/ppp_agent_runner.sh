...
if not p.exists():
    ...
    print("vite config: created")
else:
    txt = p.read_text(encoding="utf-8")
    if "/api" not in txt or "proxy" not in txt:
        import re
        txt = re.sub(r'export default defineConfig\\(\\{','export default defineConfig({\\n  server:{ proxy:{ "/api":{ target:"http://127.0.0.1:8000", changeOrigin:true, secure:false } } },',txt,1)
        p.write_text(txt, encoding="utf-8"); print("vite config: patched")
    else: print("vite config: proxy already present")
PY
    ;;
  pr5c)
